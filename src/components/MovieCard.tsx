...
export const MovieCard: React.FC<MovieCardProps> = ({ item, type, onClick, className }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative w-[140px] sm:w-[180px] md:w-[200px] aspect-[2/3] rounded-lg overflow-hidden cursor-pointer group shadow-lg bg-bg-secondary ${className}`}
      onClick={onClick}
    >
      <img
        src={item.poster_url}
      ...
      <button 
        className={`absolute top-2 right-2 z-20 p-2 rounded-full backdrop-blur-md border border-white/20 transition-all duration-300 shadow-2xl group/btn ${
          isInWatchlist ? 'bg-accent-red border-accent-red scale-110' : 'bg-black/40 hover:bg-white/20 hover:scale-110 opacity-100 sm:opacity-0 group-hover:opacity-100'
        }`}
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-4">
        <div className="flex items-center justify-center absolute inset-0 sm:opacity-100 opacity-0 group-hover:opacity-100 transition-opacity">
          <div className="w-12 h-12 rounded-full bg-accent-red flex items-center justify-center scale-0 sm:scale-0 group-hover:scale-100 transition-transform duration-300 delay-75">
